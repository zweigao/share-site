...
auto=setTimeout(lunbo,2000);                                //每2秒执行一次轮播函数，即每两秒轮播一次
}
lunbo();                                                    //一开始就执行函数，开始轮播
$btn.hover(function(){
...
function moveImg() {                       //移动图片函数
if (i == $('#ul li').length) {             //如果是最后一张的话，改变ul的样式，使他的left变为0，重新开始轮播，同时把i变成1；
$('#ul').css({
left: 0
})
i = 1;
...
$('#ul').css({
left: ($('#ul li').length - 1) * -300   //改变left的值来移动图片
});
}
